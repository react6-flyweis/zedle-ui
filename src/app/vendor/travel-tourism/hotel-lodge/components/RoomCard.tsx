...
export function RoomCard({ room, onEdit, onDelete }: RoomCardProps) {
  ...
  // Map RoomCard's room to RoomEditorDialog's expected values
  const getRoomFormValues = () => ({
    roomType: room.type,
    roomNumber: room.number,
    floorNumber: "", // Not available in RoomCard, set as empty
    width: room.size.split("x")[0]?.trim() || "",
    length: room.size.split("x")[1]?.trim() || "",
    price: "", // Not available in RoomCard, set as empty
    amenities: [], // Not available in RoomCard, set as empty
    images: [], // Not available in RoomCard, set as empty
  });
  ...
  const handleEditorSubmit = (_data: unknown) => {
    setEditorOpen(false);
    onEdit?.(room.id);
    // Optionally, pass data to parent for update
  };
